...
public class RecipeReviewServiceImplTest {
    ...
    void getAllRecipeReview() {
        given(recipeReviewRepository.findAllRecipeReviewResponse(RECIPE.getId(),pageable,orderSpecifiers))
        .willReturn(PAGING_RECIPE_REVIEW_RESPONSES);
        recipeReviewService.getRecipeReviews(RECIPE.getId(),pageable);
        verify(recipeReviewRepository, times(1)).findAllRecipeReviewResponse(RECIPE.getId(),pageable,orderSpecifiers);
    }
    @Test
    ...
    void getAllMyRecipeReview() {
        ...
        verify(recipeReviewRepository, times(1)).findAllMyRecipeReviewResponse(USER.getId(),pageable,orderSpecifiers);
    }
}
