...
def youtubeAnalysis(video_url):
  ...
  with videoinsights_client.ApiClient(configuration) as api_client:
    ...
    try:
      ...
      q = YTQuery(
          query="""
        요리 재료와 양을 표시하고, 아래 형식과 같이 요리 단계를 6개에서 10개 사이의 단계로 나누어 주세요. 각 요리 단계 내용은 자세하게 작성해주세요. 이 각 요리 단계에 맞는 유튜브 영상 시:분:초 양식의 타임라인을 포함한 JSON 형식으로 응답해 주세요. 요리가 여러 개 나오는 영상이면 대표 메뉴 하나만 응답해주세요. 최종 응답값, 필드 명은 영어로 그대로 놔두고 필드 값만 한글로 번역해서 각 항목을 채워주세요.
        {
”foodName” :String,
